...
def interpolate_grid(it, filename, M, Likelihood, bestlike, Treelist, StartTrees,  optimized_BestTrees, Topologies,NUMPATHTREES, hull_indices=None):
    ...
    ax1 = plt.subplot2grid((40,345), (14,0),rowspan=14, colspan=80)
    contour = ax1.contourf(XX, YY, values, 10, alpha=1, vmin=np.nanmin(values), vmax=np.nanmax(values), cmap='viridis')

    vmin, vmax = min(Likelihood), max(Likelihood)    #Data range :To keep consistency between colors, use the vmin, vmax arguments when breaking up the points to use colormap and different markers for different values
    
#    if it==0:
#        ax1.scatter(X[:N,0], X[:N,1], c=Likelihood[:N] ,alpha=1, marker='^', vmin = vmin, vmax = vmax, cmap='viridis',  edgecolors="black", linewidths=0.5, s=50)     #starttrees
#    if it>0:
#        ax1.scatter(X[:N-1,0], X[:N-1,1], c=Likelihood[:N-1] ,alpha=1, marker='^', vmin = vmin, vmax = vmax, cmap='viridis',  edgecolors="black", linewidths=0.5, s=35, label="Starting trees")     #starttrees without previous best tree
#        ax1.scatter(X[N-1,0], X[N-1,1], c=Likelihood[N-1] ,alpha=1, marker='^', vmin = vmin, vmax = vmax, cmap='viridis',      edgecolors="black", linewidths=1.2, s=80, label="Previous best tree \nLogLike {:4.4f}".format(Likelihood[N-1]))   #best tree from the previous iteration
    ...
    if paup_tree:
        ax1.scatter(X[-1,0],X[-1,1], marker='o',facecolors='white', edgecolors="black", linewidths=0.5, s=150)      #PAUP
        ax1.scatter(X[-2,0],X[-2,1], marker='o', facecolors='red', edgecolors="black", linewidths=0.5, s=150)     #best optimized
        opt_points = ax1.scatter(X[-opt:-1,0],X[-opt:-1,1], marker='o',c=Colors, edgecolors="black", linewidths=0.5, s=40)      #optimized trees
    else:
        ax1.scatter(X[-1,0],X[-1,1], marker='o', facecolors='red', edgecolors="black", linewidths=0.5, s=150)     #best optimized
        opt_points = ax1.scatter(X[-opt:,0],X[-opt:,1], marker='o',c=Colors, edgecolors="black", linewidths=0.5, s=40)      #optimized trees
    
    
    for j, (top, c) in enumerate(zip(Topologies, Colors)):     #colored topologies trees
    ...
    #Labels:
    smallgreen_circle = mlines.Line2D([], [], color='limegreen', marker='o', linestyle='None',mec="black", mew=0.3,
                                  markersize=3, label="{} path + {} start trees:\n{}-trees between each pair".format(all_path,N, n_path))
    green_triangle = mlines.Line2D([], [], color='limegreen', marker='^', linestyle='None',mec="black", mew=0.4,
                               markersize=8, label="{} starting trees".format(len(StartTrees)))
    smallpink_circle = mlines.Line2D([], [], color='hotpink', marker='o', linestyle='None', mec="black", mew=0.25,
                                 markersize=3, label="{} trees with highest likelihood ".format(len(bestlike)))
    bigpink_circle = mlines.Line2D([], [], color='hotpink', marker='o', linestyle='None',mec="black", mew=0.4,
                               markersize=6, label="{} best optimized tree \n of different topologies".format(N_topo))


    if paup_tree:
        bigred_circle = mlines.Line2D([], [], color='red', marker='o', linestyle='None',mec="black", mew=0.7,
                                  markersize=11, label="PATHTREES best optimized tree \nLogLike {:4.4f}".format(Likelihood[-2]))
        ...
        plt.legend(bbox_to_anchor=(1.05, 0.84),  fontsize='x-small', labelspacing = 1.5, frameon=False, scatterpoints=1, handles=[ smallgreen_circle, green_triangle, smallpink_circle, bigpink_circle,   bigred_circle, bigwhite_circle])
    else:
        ...
        plt.legend(bbox_to_anchor=(1.05, 0.84),  fontsize='x-small', labelspacing = 1.5, frameon=False, scatterpoints=1, handles=[ smallgreen_circle, green_triangle, smallpink_circle, bigpink_circle,   bigred_circle])
    ...
    surf = ax2.plot_surface(XX, YY, values, rstride=1, cstride=1, edgecolor='none',alpha=0.30, cmap='viridis',vmin=np.nanmin(values), vmax=np.nanmax(values))
    ...
    ax2.scatter3D(X[:N,0], X[:N,1], Likelihood[:N], marker='^', c=Likelihood[:N] , vmin = vmin, vmax = vmax, cmap='viridis',  edgecolors="black", linewidths=0.5, s=45)    #starttrees

#    points = ax2.scatter3D(X[N:-opt,0],X[N:-opt,1],Likelihood[N:-opt], alpha = 0.8, marker='o', c=Likelihood[N:-opt] , cmap='viridis',  edgecolors="black", linewidths=0.3, s=6)   #PathTrees
#    if it==0:
#        ax2.scatter3D(X[:N,0], X[:N,1], Likelihood[:N], marker='^', c=Likelihood[:N] , vmin = vmin, vmax = vmax, cmap='viridis',  edgecolors="black", linewidths=0.5, s=45)    #starttrees
#    if it>0:
...
#        ax2.scatter3D(X[N-1,0], X[N-1,1], Likelihood[N-1], alpha = 1, marker='^',  c=Likelihood[N-1] , vmin = vmin, vmax = vmax, cmap='viridis', edgecolors="black", linewidths=1.2, s=80)    #best tree from the previous iteration
    ...
    if paup_tree:
        ax2.scatter3D(X[-1,0],X[-1,1],Likelihood[-1], marker='o',facecolors='white', edgecolors="black", linewidths=0.5, s=150)      #PAUP
        ax2.scatter3D(X[-2,0],X[-2,1],Likelihood[-2], marker='o',facecolors='red', edgecolors="black", linewidths=0.5, s=150)     #best optimized
